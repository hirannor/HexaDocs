...
public record DocumentId(UUID uniqueComponent) {
    public DocumentId {
        if (uniqueComponent == null) throw new IllegalArgumentException("DocumentId can't be null");
    }

    public static DocumentId from(final String source) {
        if (source == null || source.isBlank()) {
            throw new IllegalArgumentException("DocumentId can't be null or empty");
        }
