...
class DocumentTextExtractionService implements DocumentTextExtracting {
  ...
  public void extract(final ExtractDocumentText command) {
    try {
      ...
      log.info("Document loaded from storage | " + "documentId={} | sizeBytes={}", command.documentId(), file.length);
      ...
      log.info("Text extraction completed | documentId={} | " + "pageCount={}", command.documentId(), pages.size());

      if (pages.isEmpty()) {
        log.info("No pages extracted | documentId={}", command.documentId());
      ...
      log.info("Saving extracted pages | documentId={} | " + "pageCount={} | totalCharacters={}", command.documentId(),
              pages.size(), totalCharacters);

      documentTextStorage.save(command.documentId(), pages);

      log.info("Publishing DocumentTextExtracted event | " + "documentId={} | ingestionJobId={}", command.documentId(),
              command.ingestionJobId());

      messages.publish(
              DocumentTextExtracted.record(command.ingestionJobId(), command.documentId(), command.knowledgeBaseId()));

      log.info("Document text extraction completed successfully | " + "documentId={}", command.documentId());

    } catch (final Exception e) {
      log.error("Document text extraction failed | documentId={}", command.documentId(), e);
