...
class DocumentVectorIndexingService implements DocumentVectorIndexing {
  ...
  private static final Logger log = LoggerFactory.getLogger(DocumentVectorIndexingService.class);
  ...
  public void index(final IndexDocument command) {
    ...
    try {
      ...
      log.info("Extracted pages loaded | documentId={} | pageCount={}", documentId.asText(), pages.size());
      ...
      log.info("Chunks created | documentId={} | count={}", documentId.asText(), chunks.size());
      ...
      log.info("Vector indexing completed | documentId={} | storedVectors={}", documentId.asText(), totalStored);
      ...
      log.error("Vector indexing failed | documentId={}", documentId.asText(), e);
